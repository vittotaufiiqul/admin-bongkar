...
export default function TabScan({ data, addRow, delRow, master, toast, setScan }) {
  ...
  // qty_terima diambil dari row.qty (field utama dari paste step 2)
  // qty_rak bisa diedit di tabel review, default = qty_terima
  // qty_lebihan = qty_terima - qty_rak (auto)
  async function handleBatchSubmit(rows) {
    const { tgl: todayTgl, wkt } = nowTs()
    let count = 0
    for (const row of rows) {
      // Baca qty dari field yang benar
      const qtyTerima  = Number(row.qty_terima || row.qty || 0)
      const qtyRak     = Number(row.qty_rak || qtyTerima) // default: semua ke rak
