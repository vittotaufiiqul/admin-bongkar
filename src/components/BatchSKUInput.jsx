      ...
      {step === 2 && (
        <div style={{ padding:'16px' }}>
          <div style={{ display:'flex', gap:16, alignItems:'flex-start' }}>
            {/* Preview SKU (read-only) */}
            <div style={{ flex:1 }}>
              <div style={{ fontSize:10, fontWeight:700, color:'var(--t3)', textTransform:'uppercase', letterSpacing:'.6px', marginBottom:6 }}>
                SKU yang diparsed ({rows.length} baris)
              </div>
              <div style={{ background:'var(--s3)', border:'1px solid var(--b1)', borderRadius:'var(--r-sm)', padding:'8px 12px', maxHeight:280, overflow:'auto' }}>
                {rows.map((r,i)=>(
                  <div key={i} style={{ display:'flex', alignItems:'center', gap:8, padding:'3px 0', borderBottom:i<rows.length-1?'1px solid var(--b0)':'none' }}>
                    <span style={{ fontSize:9, fontFamily:'var(--mono)', color:'var(--t4)', width:18, flexShrink:0 }}>{i+1}</span>
                    <span className={`badge b-sup b-${SUP_CLS[r.supplier]||'TAZ'}`} style={{ fontSize:9, padding:'1px 6px', flexShrink:0 }}>{r.supplier}</span>
                    <span style={{ fontFamily:'var(--mono)', fontSize:11, color: r.valid?'var(--amber)':'var(--red)', flex:1 }}>{r.sku}</span>
                    {r.nama && <span style={{ fontSize:10, color:'var(--t3)', overflow:'hidden', textOverflow:'ellipsis', whiteSpace:'nowrap', maxWidth:100 }}>{r.nama}</span>}
                    {!r.valid && <span style={{ fontSize:9, color:'var(--red)', fontWeight:700 }}>⚠</span>}
                  </div>
                ))}
              </div>
            </div>
            ...
            <div style={{ flex:1 }}>
              <div className="fg">
                <label><Ico.Hash/> Paste QTY ({rows.length} baris)</label>
                <textarea
                  value={qtyRaw}
                  onChange={e=>setQtyRaw(e.target.value)}
                  rows={10}
                  placeholder={'Paste QTY per baris\n(urutan harus sama dengan SKU):\n60\n40\n60\n40\n35\n...'}
                  style={{ width:'100%', background:'var(--s3)', border:'1.5px solid var(--b2)', borderRadius:'var(--r-sm)', padding:'10px 12px', color:'var(--t1)', fontFamily:'var(--mono)', fontSize:13, outline:'none', resize:'none', lineHeight:1.8, height:280 }}
                  onFocus={e=>{e.target.style.borderColor='var(--brand)';e.target.style.boxShadow='0 0 0 2.5px var(--brand-dim)'}}
                  onBlur={e=>{e.target.style.borderColor='var(--b2)';e.target.style.boxShadow='none'}}
                />
                {qtyRaw && (
                  <div style={{ fontSize:10, color:'var(--t3)', marginTop:3, fontFamily:'var(--mono)' }}>
                    {parseQTYInput(qtyRaw).length} baris QTY
                    ...
                    {parseQTYInput(qtyRaw).length === rows.length && (
                      <span style={{ color:'var(--green)', marginLeft:8, fontWeight:700 }}>✓ Jumlah sesuai</span>
                    )}
                  </div>
                )}
              </div>
