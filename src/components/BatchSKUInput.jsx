...
export default function BatchSKUInput({ mode, master, onSubmit, toast, onClose, tgl }) {
  ...
  // ── Step panel (JSX variable for reuse) ──────────────────────
  const StepsPanel = (
      ...
      <>
        <div style={{ padding:'14px 16px' }}>
          <div style={{ display:'flex', alignItems:'center', gap:10, marginBottom:12 }}>
            <StepNum n={4} status="active"/>
            <div>
              <div style={{ fontSize:13, fontWeight:700, color:'var(--t1)' }}>Review & Simpan</div>
              <div style={{ fontSize:11, color:'var(--t3)' }}>Review data sebelum disimpan ke sistem</div>
            </div>
          </div>
          ...
          <div style={{ display:'flex', gap:8 }}>
            <button onClick={()=>setStep(3)} className="btn btn-ghost btn-sm">← Edit QTY</button>
            <button onClick={()=>setOverlayOpen(true)}
              className="btn btn-primary" style={{ flex:1, justifyContent:'center', gap:6 }}>
              <IcoBox/> Buka Review Lengkap →
            </button>
          </div>
        </div>
        {/* Fullscreen overlay for step 4 */}
        {overlayOpen && (
          <div style={{
            ...
            {/* Overlay header */}
            <div style={{ background:'var(--s1)', borderBottom:'1px solid var(--b1)', padding:'0 24px', height:52, display:'flex', alignItems:'center', gap:12, flexShrink:0 }}>
              <div style={{ width:32,height:32,background:'var(--brand)',borderRadius:8,display:'flex',alignItems:'center',justifyContent:'center',color:'#fff',flexShrink:0 }}>
                <IcoBox/>
              </div>
              <div>
                <div style={{ fontSize:14,fontWeight:700,color:'var(--t1)' }}>Review & Simpan — Scan Masuk</div>
                <div style={{ fontSize:11,color:'var(--t3)' }}>Periksa kembali data sebelum disimpan ke sistem</div>
              </div>
              <button onClick={()=>setOverlayOpen(false)} className="btn btn-ghost btn-sm" style={{ marginLeft:'auto' }}>
                ✕ Tutup
              </button>
            </div>

            {/* Overlay body: left steps + right review */}
            <div style={{ display:'flex', flex:1, minHeight:0 }}>
              {/* Left: steps */}
              <div style={{ width:300, flexShrink:0, borderRight:'1px solid var(--b1)', overflowY:'auto', background:'var(--s1)' }}>
                <div style={{ padding:'16px' }}>
                  {/* Compact step summary */}
                  {[
                    {n:1, label:'Tanggal Barang',  sub:tgl,            status:'done'},
                    {n:2, label:'Paste SKU',        sub:rows.length+' SKU', status:'done'},
                    {n:3, label:'Paste QTY Terima', sub:totalQty.toLocaleString('id')+' pcs', status:'done'},
                    {n:4, label:'Review & Simpan',  sub:'Sedang ditinjau', status:'active'},
                  ].map((s,i)=>(
                    <div key={s.n} style={{ display:'flex', alignItems:'flex-start', gap:10, marginBottom:i<3?20:0 }}>
                      <div style={{ display:'flex', flexDirection:'column', alignItems:'center' }}>
                        <StepNum n={s.n} status={s.status}/>
                        {i<3&&<div style={{ width:1,height:20,background:'var(--b2)',marginTop:4 }}/>}
                      </div>
                      <div style={{ paddingTop:6 }}>
                        <div style={{ fontSize:12,fontWeight:700,color:s.status==='active'?'var(--t1)':'var(--t2)' }}>{s.label}</div>
                        <div style={{ fontSize:11,color:s.status==='done'?'var(--green-lt)':'var(--t3)',marginTop:2 }}>{s.sub}</div>
                      </div>
                    </div>
                  ))}

                  {/* Mini stats */}
                  <div style={{ marginTop:20, paddingTop:16, borderTop:'1px solid var(--b1)', display:'grid', gridTemplateColumns:'1fr 1fr', gap:8 }}>
                    {[
                      {l:'Total SKU',   v:totalSKU,                              c:'var(--brand-lt)'},
                      {l:'Total Terima',v:totalQty.toLocaleString('id')+' pcs',  c:'var(--t1)'},
                      ...(mode==='scan'?[
                        {l:'→ Rak',     v:totalRak.toLocaleString('id')+' pcs',  c:'var(--green-lt)'},
                        {l:'→ Lebihan', v:totalLebihan.toLocaleString('id')+' pcs',c:'var(--orange-lt)'},
              ...
              <div style={{ flex:1, minWidth:0, display:'flex', flexDirection:'column', overflow:'hidden' }}>
                <div style={{ flex:1, overflowY:'auto', padding:'20px 24px' }}>
                  ...
                  {/* Summary numbers */}
                  <div style={{ display:'grid', gridTemplateColumns:mode==='scan'?'repeat(5,1fr)':'1fr 1fr', gap:16, background:'var(--s2)', border:'1px solid var(--b1)', borderRadius:'var(--r)', padding:'16px', marginBottom:16 }}>
                    {[
                  ...
                  {mode==='scan'&&(
                    <div style={{ display:'flex', gap:10, marginBottom:16, flexWrap:'wrap' }}>
                      <div style={{ flex:1, minWidth:260 }}>
                        <BatchPastePanel label="QTY Terima" hint={`${totalSKU} baris`} count={totalSKU} onApply={applyBatchTerima}/>
                      </div>
                      <div style={{ flex:1, minWidth:260 }}>
                        <BatchPastePanel label="QTY ke Rak" hint={`${totalSKU} baris`} count={totalSKU} onApply={applyBatchRak}/>
                      </div>
                    </div>
                  )}
                  ...
                  <div style={{ background:'var(--s1)',border:'1px solid var(--b1)',borderRadius:'var(--r)',overflow:'hidden',marginBottom:16 }}>
                    <div style={{ padding:'10px 16px',background:'var(--s2)',borderBottom:'1px solid var(--b1)',display:'flex',alignItems:'center',gap:8 }}>
                      <div style={{ fontSize:13,fontWeight:700,color:'var(--t1)',display:'flex',alignItems:'center',gap:7 }}><IcoBox/> Detail SKU ({rows.length})</div>
                      <div style={{ fontSize:11,color:'var(--t3)',marginLeft:'auto' }}>Klik nilai QTY untuk edit</div>
                    </div>
                    <div style={{ overflowY:'auto', maxHeight:440 }}>
                      <table style={{ width:'100%',borderCollapse:'collapse',fontSize:12,tableLayout:'fixed' }}>
                        <colgroup>
                          ...
                          <col style={{ width:80 }}/>
                          <col style={{ width:80 }}/>
                          {mode==='scan'&&<col style={{ width:72 }}/>}
                          {mode==='perm'&&<col style={{ width:96 }}/>}
                          <col style={{ width:30 }}/>
                          <col style={{ width:30 }}/>
                        </colgroup>
                        <thead style={{ position:'sticky',top:0,zIndex:10 }}>
                          <tr>
                            {['#','SKU','Nama Produk','Rak',
                              ...
                              <th key={i} style={{ background:'var(--s2)',padding:'9px 10px',textAlign:['Terima','QTY','→ Rak','→ Lebihan'].includes(h)?'center':'left',fontSize:11,fontWeight:600,color:'var(--t3)',whiteSpace:'nowrap',borderBottom:'1px solid var(--b1)' }}>
                                {h}
                              </th>
                            ))}
                          </tr>
                          ...
                          {rows.map(row=>{
                            ...
                            return (
                              <tr key={row.idx} style={{ borderBottom:'1px solid var(--b0)',background:!row.valid?'rgba(239,68,68,.03)':undefined }}>
                                <td style={{ padding:'8px 10px',color:'var(--t4)',fontFamily:'var(--mono)',fontSize:11,textAlign:'center' }}>{row.idx+1}</td>
                                <td style={{ padding:'8px 10px' }}>
                                  <div style={{ fontFamily:'var(--mono)',fontSize:11,fontWeight:600,color:row.valid?'var(--amber-lt)':'var(--red-lt)',overflow:'hidden',textOverflow:'ellipsis',whiteSpace:'nowrap',marginBottom:3 }}>{row.sku}</div>
                                  <span className={`badge b-sup b-${SUP_CLS[row.supplier]||'TAZ'}`} style={{ fontSize:9,padding:'1px 5px' }}>{row.supplier}</span>
                                </td>
                                <td style={{ padding:'8px 10px' }}>
                                  {row.fromMaster
                                    ?<span title={row.nama} style={{ fontSize:12,color:'var(--t1)',fontWeight:500,display:'block',overflow:'hidden',textOverflow:'ellipsis',whiteSpace:'nowrap' }}>{row.nama||'—'}</span>
                                    :<input value={row.nama||''} onChange={e=>updateRow(row.idx,'nama',e.target.value)} placeholder="Isi nama..."
                                        style={{ width:'100%',background:'var(--s4)',border:'1px solid var(--amber)',borderRadius:4,padding:'3px 7px',color:'var(--t1)',fontFamily:'var(--font)',fontSize:11,outline:'none' }}/>
                                  }
                                </td>
                                <td style={{ padding:'8px 10px',textAlign:'center' }}>
                                  <span style={{ fontFamily:'var(--mono)',fontSize:12,fontWeight:700,color:'var(--cyan-lt)' }}>{row.rak||'—'}</span>
                                </td>
                                <td style={{ padding:'5px 8px',textAlign:'center' }}>
                                  <QtyCell value={mode==='scan'?(row.qty_terima||row.qty||''):(row.qty||'')}
                                    onChange={v=>updateRow(row.idx,mode==='scan'?'qty_terima':'qty',v)}/>
                                </td>
                                {mode==='scan'?(
                                  <td style={{ padding:'5px 8px',textAlign:'center' }}>
                                    <QtyCell value={row.qty_rak||''} onChange={v=>updateRow(row.idx,'qty_rak',v)} color="var(--green-lt)"/>
                                  </td>
                                ):(
                                  <td style={{ padding:'5px 8px' }}>
                                    <select value={row.jenis||'rak'} onChange={e=>updateRow(row.idx,'jenis',e.target.value)}
                                      style={{ width:'100%',background:'var(--s4)',border:'1px solid var(--b2)',borderRadius:4,padding:'5px 4px',color:'var(--t1)',fontSize:10,outline:'none',fontFamily:'var(--font)' }}>
                                ...
                                {mode==='scan'&&(
                                  <td style={{ padding:'8px 10px',textAlign:'center',fontFamily:'var(--mono)',fontSize:13,fontWeight:700,color:lbh>0?'var(--orange-lt)':'var(--t4)' }}>{lbh>0?lbh:'0'}</td>
                                )}
                ...
                {/* Sticky bottom bar */}
                <div className="sticky-bar" style={{ borderTop:'1px solid var(--b1)' }}>
                  ...
                  <button onClick={()=>setOverlayOpen(false)} className="btn btn-ghost">Kembali</button>
                  <button onClick={onClose} className="btn btn-ghost" style={{ color:'var(--red-lt)' }}>Batal</button>
