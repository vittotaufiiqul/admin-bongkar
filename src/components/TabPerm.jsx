...
export default function TabPerm({ data, addRow, delRow, master, toast, scan, pindahList }) {
  ...
  // Cek stok lebihan untuk SKU yang dipilih
  const stokLebihan = useMemo(() => {
    if (!f.fullSku || !scan?.length) return 0
    const totalMasuk = scan
      .filter(s => s.sku === f.fullSku && s.supplier === f.sup)
      .reduce((a, s) => a + Number(s.qty_lebihan || 0), 0)
    const totalPindah = (pindahList || [])
      .filter(p => p.sku === f.fullSku && p.supplier === f.sup)
      .reduce((a, p) => a + Number(p.qty_pindah), 0)
    return Math.max(0, totalMasuk - totalPindah)
  }, [f.fullSku, f.sup, scan, pindahList])
        ...
        <div>
          <div className="card">
            ...
            <div className="card-body">
              ...
              {/* Notifikasi stok lebihan */}
              {stokLebihan > 0 && (
                <div className="notif orange" style={{ marginBottom: 10 }}>
                  📦 <div>
                    <strong>Stok Lebihan Tersedia: {stokLebihan} pcs</strong>
                    <div style={{ marginTop: 3, fontSize: 11 }}>
                      SKU ini masih ada <strong>{stokLebihan} pcs</strong> di area lebihan.
                      Pertimbangkan ambil dari lebihan dulu — cek tab <strong>🗄️ Lebihan & Rak</strong>.
                    </div>
                  </div>
                </div>
              )}
              ...
              <div className="fg-row col2">
                ...
                <div className="fg">
                  ...
                  <input ref={qtyRef} type="number" min={1} className="mono" value={qty}
                    ...
                    onKeyDown={e => { if (e.key === 'Enter') add() }} placeholder="0" />
                </div>
              </div>
