 ...
 *
 * Flow:
 * 1. Ketik nomor karung → Buka Karung
 * 2. Scan SKU satu per satu ke dalam karung (bisa beberapa SKU)
 * 3. Edit qty tiap item jika perlu
 * 4. Simpan semua sekaligus
 *
 * Bisa buka beberapa karung paralel.
 * Satu SKU bisa masuk ke beberapa karung berbeda.
 */
...
// ─────────────────────────────────────────────────────────────
// CartItem — satu baris SKU dalam karung (editable qty)
// ─────────────────────────────────────────────────────────────
function CartItem({ item, index, onQtyChange, onRemove }) {
  ...
  return (
    <div style={{
      ...
      <span style={{ fontFamily:'var(--mono)', fontSize:11, color:'var(--amber)', flexShrink:0 }}>{item.sku}</span>
      <span style={{ fontSize:12, color:'var(--t1)', flex:1, overflow:'hidden', textOverflow:'ellipsis', whiteSpace:'nowrap' }}>
        {item.nama || '—'}
      </span>
      {item.rak && (
        <span style={{ fontSize:10, color:'var(--cyan)', fontFamily:'var(--mono)', flexShrink:0 }}>Rak {item.rak}</span>
      )}
      ...
      {editing ? (
        <div style={{ display:'flex', alignItems:'center', gap:4, flexShrink:0 }}>
          <input ref={inputRef} type="number" min={1} value={editVal}
            ...
            onKeyDown={e => {
              if (e.key==='Enter')  { onQtyChange(editVal); setEditing(false) }
              if (e.key==='Escape') { setEditing(false); setEditVal(String(item.qty)) }
            }}
            inputMode="numeric"
          ...
          <button onClick={()=>{ onQtyChange(editVal); setEditing(false) }}
            style={{ background:'var(--green)',border:'none',borderRadius:5,padding:'4px 8px',cursor:'pointer',fontSize:11,fontWeight:700,color:'#000',minHeight:28 }}>✓</button>
          <button onClick={()=>{ setEditing(false); setEditVal(String(item.qty)) }}
            style={{ background:'var(--s4)',border:'1px solid var(--b2)',borderRadius:5,padding:'4px 7px',cursor:'pointer',fontSize:11,color:'var(--t2)',minHeight:28 }}>✕</button>
        </div>
      ) : (
        <div style={{ display:'flex', alignItems:'center', gap:5, flexShrink:0 }}>
          <span style={{ fontFamily:'var(--mono)', fontSize:16, fontWeight:800, color:'var(--green)', minWidth:34, textAlign:'right' }}>{item.qty}</span>
          <span style={{ fontSize:10, color:'var(--t3)' }}>pcs</span>
          <button onClick={()=>{ setEditing(true); setTimeout(()=>inputRef.current?.select(),40) }}
            className="del"><Ico.Edit/></button>
        </div>
      )}
...
// ─────────────────────────────────────────────────────────────
// KarungCart — satu karung yang sedang diisi
// ─────────────────────────────────────────────────────────────
function KarungCart({ karung, scan, pindahList, karunginList, master, onSaveKarung, onRemoveKarung, toast }) {
  ...
  // Sisa lebihan SKU (dikurangi cart + riwayat)
  function calcSisa(supplier, sku, excludeCartQty = 0) {
    let total = 0
    ...
    total += excludeCartQty // untuk tampilkan sisa sebelum qty yang baru diisi
  ...
  function addItem() {
    ...
    if (Number(qty) > sisaSebelumQty && sisaSebelumQty > 0) {
      toast(`Maks ${sisaSebelumQty} pcs sisa lebihan.`, false); return
    }

    const existing = items.find(i => i.supplier===f.sup && i.sku===f.fullSku)
    if (existing) {
      // SKU sudah ada → tambah qty
      setItems(prev => prev.map(i =>
        i.supplier===f.sup && i.sku===f.fullSku
          ? { ...i, qty: i.qty + Number(qty) }
          : i
      ))
    } else {
      setItems(prev => [...prev, {
        supplier: f.sup, sku: f.fullSku,
        nama: f.nama, rak: f.rak,
        qty: Number(qty),
      }])
    }
  ...
  function removeItem(supplier, sku) {
    setItems(prev => prev.filter(i => !(i.supplier===supplier && i.sku===sku)))
  }

  function updateQty(supplier, sku, val) {
    setItems(prev => prev.map(i =>
      i.supplier===supplier && i.sku===sku ? { ...i, qty: Number(val)||0 } : i
    ))
  }

  async function saveAll() {
    if (items.length === 0) { toast('Karung masih kosong!', false); return }
    if (items.some(i => !i.qty || i.qty <= 0)) { toast('Ada QTY yang 0!', false); return }
    setSaving(true)
    try {
      await onSaveKarung(karung.nomor, items)
      setItems([])
      toast(`Karung ${karung.nomor} (${items.length} SKU) disimpan.`)
    ...
    <div className="card" style={{ marginBottom:12, borderColor: items.length>0?'var(--brand)':'var(--b1)' }}>
      ...
      <div className="card-hdr" style={{ justifyContent:'space-between' }}>
        <div style={{ display:'flex', alignItems:'center', gap:8 }}>
          ...
          <span style={{ fontFamily:'var(--mono)', fontWeight:800, fontSize:15, color:'var(--brand-lt)', letterSpacing:'1.5px' }}>
            {karung.nomor}
          </span>
          {items.length > 0 && (
            <span className="n-badge">{items.length} SKU · {totalQty} pcs</span>
          )}
        </div>
        <div style={{ display:'flex', gap:6 }}>
          <button onClick={()=>setScanOpen(v=>!v)} className="btn btn-ghost btn-xs">
            {scanOpen ? '▲ Tutup' : '▼ Scan SKU'}
          </button>
          {items.length === 0 && (
            <button onClick={()=>onRemoveKarung(karung.id)} className="btn btn-ghost btn-xs" style={{ color:'var(--red)' }}>
              <Ico.X/> Hapus Karung
            </button>
          )}
        </div>
      ...
      {scanOpen && (
        <div style={{ padding:'12px 14px', background:'var(--s3)', borderBottom:'1px solid var(--b1)' }}>
          ...
          {/* SKU + QTY + Tambah dalam satu baris */}
          <div style={{ display:'flex', gap:8, alignItems:'flex-end' }}>
            <div style={{ flex:2 }}>
              <div style={{ fontSize:10, fontWeight:700, color:'var(--t2)', textTransform:'uppercase', letterSpacing:'.6px', marginBottom:4 }}>
                SKU <span style={{ fontWeight:400, textTransform:'none', letterSpacing:0, color:'var(--t3)' }}>4 digit terakhir</span>
              </div>
            ...
            <div style={{ flex:1 }}>
              ...
              <input
                ref={qtyRef} type="number" min={1} value={qty}
            ...
            <button onClick={addItem} disabled={f.suffix.length!==4||!qty} className="btn btn-primary"
              style={{ flexShrink:0, minHeight:36, padding:'0 14px' }}>
              <Ico.Plus/> Tambah
            </button>
          </div>
          ...
          {f.fullSku && (
            <div style={{ marginTop:8, display:'flex', gap:10, flexWrap:'wrap', alignItems:'center', fontSize:11 }}>
              ...
              {f.rak  && <span style={{ color:'var(--cyan)', fontFamily:'var(--mono)' }}>Rak {f.rak}</span>}
              {f.ls==='found'    && <span style={{ color:'var(--green)',  fontWeight:700, display:'flex', alignItems:'center', gap:3 }}><Ico.Check/> Di master</span>}
              {f.ls==='notfound' && <span style={{ color:'var(--amber)', fontWeight:700 }}>⚠ Tidak di master</span>}
              <span style={{ marginLeft:'auto', display:'flex', alignItems:'center', gap:4, color: sisaSebelumQty>0?'var(--orange)':'var(--t3)', fontWeight:600 }}>
                <Ico.Package/>
                {sisaSebelumQty > 0 ? `Sisa lebihan: ${sisaSebelumQty} pcs` : 'Tidak ada di lebihan'}
              </span>
            </div>
          )}
          {f.fullSku && sisaSebelumQty > 0 && (
            <button onClick={()=>setQty(String(sisaSebelumQty))}
              style={{ marginTop:4, fontSize:9, color:'var(--orange)', background:'none', border:'none', cursor:'pointer', padding:0, fontFamily:'var(--font)', display:'flex', alignItems:'center', gap:3 }}>
              <Ico.Info/> Pakai semua sisa ({sisaSebelumQty} pcs)
            </button>
          )}
        </div>
      )}
      ...
      {/* Daftar isi karung */}
      {items.length === 0 ? (
        <div style={{ padding:'18px 14px', textAlign:'center', color:'var(--t3)', fontSize:12 }}>
          Belum ada SKU — scan di atas untuk menambah
        </div>
      ) : (
        <>
          ...
          <div style={{ padding:'12px 14px', background:'var(--s2)', borderTop:'1px solid var(--b1)', display:'flex', alignItems:'center', gap:12 }}>
            <div style={{ flex:1, fontSize:12 }}>
              ...
              <strong style={{ color:'var(--green)', fontFamily:'var(--mono)', fontSize:14 }}>{totalQty}</strong>
              <span style={{ color:'var(--t3)' }}> pcs total</span>
            </div>
...
export default function TabKarung({ scan, pindahList, karunginList, addKarungin, delKarungin, master, toast }) {
  ...
  function bukaKarung() {
    ...
    if (!nomor) { toast('Masukkan nomor karung!', false); return }
    if (karungList.find(k => k.nomor === nomor)) { toast(`Karung ${nomor} sudah terbuka!`, false); return }
  ...
  const lebihanArr = useMemo(() => {
    const map = {}
    scan.forEach(s => {
      const l = Number(s.qty_lebihan||0); if(l<=0) return
      const k = `${s.supplier}__${s.sku}`
      if (!map[k]) map[k] = { supplier:s.supplier, sku:s.sku, nama:s.nama||'', total:0 }
      map[k].total += l
    })
    ...
    <div>
      ...
      <div className="stats" style={{ marginBottom:12 }}>
        {[
          ...
          { l:'Total Pcs Lebihan',    v:totalLebihan,         c:'var(--amber)'    },
      ...
      <div className="card" style={{ marginBottom:12 }}>
        <div className="card-hdr"><Ico.Plus/> Buka Karung</div>
        <div className="card-body">
          <div style={{ display:'flex', gap:8, alignItems:'flex-end' }}>
            <div style={{ flex:1 }}>
              <div style={{ fontSize:10, fontWeight:700, color:'var(--t2)', textTransform:'uppercase', letterSpacing:'.6px', marginBottom:5 }}>Nomor Karung</div>
              <input
                ...
                style={{ width:'100%', background:'var(--s3)', border:'1.5px solid var(--b2)', borderRadius:'var(--r-sm)', padding:'9px 12px', color:'var(--t1)', fontFamily:'var(--mono)', fontSize:16, fontWeight:700, letterSpacing:'1px', outline:'none' }}
                onFocus={e=>{e.target.style.borderColor='var(--brand)';e.target.style.boxShadow='0 0 0 2.5px var(--brand-dim)'}}
                onBlur={e=>{e.target.style.borderColor='var(--b2)';e.target.style.boxShadow='none'}}
              />
            </div>
            <button onClick={bukaKarung} className="btn btn-primary" style={{ minHeight:40, padding:'0 20px', flexShrink:0 }}>
              <Ico.Box/> Buka Karung
            </button>
          </div>
              ...
              <div style={{ display:'flex', gap:5, flexWrap:'wrap' }}>
                {lebihanArr.map(item=>(
                  <div key={`${item.supplier}__${item.sku}`}
                    ...
                    <span style={{ fontFamily:'var(--mono)', fontSize:10, color:'var(--amber)' }}>{item.sku.slice(-4)}</span>
                    <span style={{ color:'var(--t2)' }}>{item.nama}</span>
                    <span style={{ fontFamily:'var(--mono)', fontWeight:800, color:'var(--orange)' }}>{item.total}</span>
                  </div>
                ))}
              </div>
            </div>
      ...
      {karungList.length === 0 ? (
        ...
        <KarungCart
          ...
          karunginList={karunginList}
          master={master}
          onSaveKarung={saveKarung}
      ...
      <div className="dp-bar" style={{ flexWrap:'wrap', gap:8, marginTop:4 }}>
        <div style={{ display:'flex', alignItems:'center', gap:6 }}>
          <Ico.History/>
          <span style={{ fontSize:11, fontWeight:700, color:'var(--t2)' }}>Riwayat</span>
        </div>
            ...
            return (
              <div key={nomor} style={{ borderBottom:'1px solid var(--b0)' }}>
                <div className={`group-hdr ${isOpen?'open':''}`} onClick={()=>toggle('k_'+nomor)}>
                  <div style={{ width:28,height:28,background:'var(--brand-dim)',border:'1px solid var(--brand-glow)',borderRadius:7,display:'flex',alignItems:'center',justifyContent:'center',color:'var(--brand-lt)',flexShrink:0 }}>
                    <Ico.Box/>
                  </div>
                  ...
                  <span style={{ fontSize:11,color:'var(--t2)' }}>
                    <strong style={{ color:'var(--green)',fontFamily:'var(--mono)' }}>{totalQty}</strong> pcs
                  </span>
                  ...
                  <div style={{ padding:'8px 14px', background:'var(--s2)' }}>
                    {items.map(row=>(
                      <div key={row.id} style={{ display:'flex',alignItems:'center',gap:10,padding:'7px 12px',background:'var(--s1)',border:'1px solid var(--b1)',borderRadius:'var(--r-sm)',marginBottom:5 }}>
                        ...
                        {row.rak&&<span style={{ fontSize:10,color:'var(--cyan)',fontFamily:'var(--mono)',flexShrink:0 }}>Rak {row.rak}</span>}
                        <span style={{ fontFamily:'var(--mono)',fontSize:15,fontWeight:800,color:'var(--green)',flexShrink:0 }}>{row.qty} pcs</span>
                    ...
                    <div style={{ textAlign:'right',fontSize:11,color:'var(--t3)',paddingTop:4 }}>
                      Total: <strong style={{ fontFamily:'var(--mono)',color:'var(--green)' }}>{totalQty} pcs</strong>
                    </div>
                  </div>
                )}
              </div>
          ...
          return (
            <div key={tgl} className="card" style={{ marginBottom:8 }}>
              <div className={`group-hdr ${isOpen?'open':''}`} onClick={()=>toggle(tgl)}>
                ...
                <span style={{ fontSize:10,color:'var(--t2)' }}>Total: <strong style={{ color:'var(--green)' }}>{ttl} pcs</strong></span>
                <span className="group-chevron">▼</span>
              </div>
              {isOpen&&(
