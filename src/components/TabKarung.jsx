 ...
 * Kode karung auto-generate: TAZ-AG01, ORI-AG02, dll.
 * Format: {BRAND}-{TAHUN_HURUF}{BULAN_HURUF}{URUTAN_2DIGIT}
 */
...
import { generateKodeKarung, decodeKodeKarung, getKodeBulanTahun, BRAND_PREFIX } from '../lib/karungCode'
...
function KarungCart({ karung, scan, pindahList, karunginList, master, onSaveKarung, onRemoveKarung, toast }) {
  ...
  async function saveAll() {
    ...
    setSaving(true)
    try { await onSaveKarung(karung.nomor,items); setItems([]); toast(`Karung ${karung.nomor} disimpan.`) }
    catch(e){toast('Gagal: '+e.message,false)}
    setSaving(false)
  }

  const totalQty = items.reduce((a,i)=>a+Number(i.qty||0),0)
  const decoded  = decodeKodeKarung(karung.nomor)
    ...
    <div className="card" style={{ marginBottom:12, borderColor:items.length>0?'var(--brand)':'var(--b1)' }}>
      <div className="card-hdr" style={{ justifyContent:'space-between' }}>
        <div style={{ display:'flex', alignItems:'center', gap:8 }}>
          ...
          <div>
            <div style={{ fontFamily:'var(--mono)', fontWeight:800, fontSize:16, color:'var(--brand-lt)', letterSpacing:'2px' }}>{karung.nomor}</div>
            <div style={{ fontSize:10, color:'var(--t3)', marginTop:1 }}>{decoded}</div>
          </div>
          {items.length>0&&<span className="n-badge" style={{ marginLeft:4 }}>{items.length} SKU · {totalQty} pcs</span>}
        </div>
      ...
      {scanOpen&&(
        <div style={{ padding:'12px 14px', background:'var(--s3)', borderBottom:'1px solid var(--b1)' }}>
          ...
          {/* Supplier sudah dikunci sesuai brand karung */}
          <div style={{ marginBottom:10, display:'flex', alignItems:'center', gap:8 }}>
            <span className={`badge b-sup b-${SUP_CLS[karung.supplier]}`} style={{ fontSize:12, padding:'4px 12px' }}>{karung.supplier}</span>
            <span style={{ fontSize:11, color:'var(--t3)' }}>— karung ini hanya untuk {karung.supplier}</span>
          </div>
          <div style={{ display:'flex', gap:8, alignItems:'flex-end' }}>
            <div style={{ flex:2 }}>
              ...
              <div className="sku-group">
                ...
                <input ref={f.suffixRef}
                  ...
                  autoComplete="off" inputMode="numeric" style={{ fontSize:20 }}
            ...
            <button onClick={addItem} disabled={f.suffix.length!==4||!qty}
              className="btn btn-primary" style={{ flexShrink:0, minHeight:36, padding:'0 14px' }}>
              <Ico.Plus/> Tambah
            </button>
          </div>
          {f.fullSku&&(
...
export default function TabKarung({ scan, pindahList, karunginList, addKarungin, delKarungin, master, toast }) {
  const [karungList,  setKarungList]  = useState([]) // karung aktif sesi ini
  ...
  // Kode bulan-tahun saat ini, contoh: "AG"
  const kodeBT = getKodeBulanTahun()

  // Preview kode berikutnya per supplier
  const previewKode = useMemo(()=>{
    const result = {}
    SUPPLIERS.forEach(s => { result[s] = generateKodeKarung(s, karunginList) })
    return result
  }, [karunginList])

  // Semua nomor karung yang sudah terpakai (untuk validasi)
  const nomorTerpakai = useMemo(()=>
    new Set(karunginList.map(k=>k.nomor_karung)),
    [karunginList]
  )

  function bukaKarung(supplier) {
    const nomor = generateKodeKarung(supplier, karunginList)

    // Double check: tidak boleh sudah terpakai (edge case)
    if (nomorTerpakai.has(nomor)) {
      toast(`Kode ${nomor} sudah dipakai — refresh halaman dan coba lagi.`, false)
      return
    }

    // Tidak boleh duplikat di sesi aktif
    if (karungList.find(k=>k.nomor===nomor)) {
      toast(`Karung ${nomor} sudah terbuka di sesi ini.`, false)
      return
    }

    setKarungList(prev=>[...prev, { id:Date.now(), nomor, supplier }])
  }
  ...
  const SUP_COLOR = {
    Tazbiya:'var(--cyan-lt)', Oriana:'var(--amber-lt)',
    Zianisa:'var(--green-lt)', Baneska:'var(--purple-lt)'
  }
    ...
    <div>
      ...
      <div className="stats" style={{ marginBottom:12 }}>
        {[
          ...
          {l:'Total Karung Dibuat',  v:nomorTerpakai.size,    c:'var(--t2)'       },
      ...
      {/* ── Buka karung baru — pilih brand, kode auto-generate ── */}
      <div className="card" style={{ marginBottom:12 }}>
        <div className="card-hdr"><Ico.Plus/> Buat Karung Baru</div>
        <div className="card-body">
          ...
          <div className="info-box blue" style={{ marginBottom:14 }}>
            <Ico.Info/>
            <div>
              Kode karung otomatis: <strong>{SUPPLIERS.map(s=>`${BRAND_PREFIX[s]}-${kodeBT}01`)[0]}</strong> dst.
              Format: <strong>BRAND-{kodeBT.split('').join('')}{'{URUTAN}'}</strong>
              — huruf pertama = tahun, huruf kedua = bulan, nomor urut per brand per bulan.
            </div>
          </div>

          {/* Pilih brand → kode langsung muncul */}
          <div style={{ display:'grid', gridTemplateColumns:'repeat(auto-fit, minmax(160px, 1fr))', gap:10 }}>
            {SUPPLIERS.map(supplier => {
              const kode   = previewKode[supplier]
              const active = karungList.find(k=>k.supplier===supplier)
              const decoded = decodeKodeKarung(kode)

              return (
                <button key={supplier} onClick={()=>bukaKarung(supplier)}
                  disabled={!!active}
                  style={{
                    background: active ? 'var(--s3)' : 'var(--s2)',
                    border: `1.5px solid ${active ? 'var(--brand)' : 'var(--b2)'}`,
                    borderRadius: 'var(--r)',
                    padding: '14px 16px',
                  ...
                  onMouseEnter={e=>{ if(!active){ e.currentTarget.style.borderColor='var(--brand)'; e.currentTarget.style.background='var(--s3)' }}}
                  onMouseLeave={e=>{ if(!active){ e.currentTarget.style.borderColor='var(--b2)'; e.currentTarget.style.background='var(--s2)' }}}>

                  {/* Supplier name */}
                  <div style={{ fontSize:13, fontWeight:800, color:SUP_COLOR[supplier], marginBottom:6 }}>{supplier}</div>

                  {/* Kode yang akan dibuat */}
                  <div style={{ fontFamily:'var(--mono)', fontSize:18, fontWeight:800, color:'var(--brand-lt)', letterSpacing:'2px', marginBottom:4 }}>
                    {kode}
                  </div>

                  {/* Decode */}
                  <div style={{ fontSize:10, color:'var(--t3)', lineHeight:1.4 }}>{decoded}</div>

                  {/* Status */}
                  {active ? (
                    <div style={{ marginTop:8, fontSize:10, color:'var(--brand-lt)', fontWeight:700, display:'flex', alignItems:'center', gap:4 }}>
                      <Ico.Check/> Karung ini sudah terbuka
                    </div>
                  ) : (
                    <div style={{ marginTop:8, fontSize:10, color:'var(--t3)', display:'flex', alignItems:'center', gap:4 }}>
                      <Ico.Plus/> Klik untuk buka karung ini
                    </div>
                  )}
                </button>
              )
          ...
          {lebihanArr.length>0&&(
            <div style={{ marginTop:14 }}>
              <div style={{ fontSize:10, color:'var(--t3)', fontWeight:700, textTransform:'uppercase', letterSpacing:'.6px', marginBottom:6 }}>
                Stok lebihan ({lebihanArr.length} SKU · {totalLebihan} pcs)
              </div>
      ...
      {karungList.length===0?(
        <div className="card" style={{ marginBottom:12 }}>
          <div style={{ padding:'24px', textAlign:'center', color:'var(--t3)', fontSize:12 }}>
            Pilih brand di atas untuk membuat karung baru
          </div>
        </div>
            ...
            return (
              <div key={nomor} style={{ borderBottom:'1px solid var(--b0)' }}>
                <div className={`group-hdr ${isOpen?'open':''}`} onClick={()=>toggle('k_'+nomor)}>
                  ...
                  <div>
                    <div style={{ fontFamily:'var(--mono)',fontSize:14,fontWeight:800,color:'var(--brand-lt)',letterSpacing:'1.5px' }}>{nomor}</div>
                    <div style={{ fontSize:9,color:'var(--t3)',marginTop:1 }}>{decodeKodeKarung(nomor)}</div>
                  </div>
                    ...
                    <tbody>
                      {rows.map(row=>(
                        <tr key={row.id}>
                          ...
                          <td>
                            <div style={{ fontFamily:'var(--mono)',fontSize:13,fontWeight:800,color:'var(--brand-lt)',letterSpacing:'1px' }}>{row.nomor_karung}</div>
                            <div style={{ fontSize:9,color:'var(--t3)',marginTop:1 }}>{decodeKodeKarung(row.nomor_karung)}</div>
                          </td>
