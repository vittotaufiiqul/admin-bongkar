...
export default function TabRekap({ perm, scan, toast }) {
  ...
  // single mode when from === to, range mode when different
      ...
      <div className="dp-bar">
        ...
        {!isSingle && activeDates.length > 0 && (
          <div style={{ display: 'flex', gap: 4, flexWrap: 'wrap', alignItems: 'center' }}>
            <span style={{ fontSize: 11, color: 'var(--t2)' }}>Aktif:</span>
            {activeDates.slice(0, 5).map(d => <span key={d} className="badge b-info" style={{ fontSize: 10 }}>{d}</span>)}
            {activeDates.length > 5 && <span style={{ fontSize: 10, color: 'var(--t3)' }}>+{activeDates.length - 5}</span>}
          </div>
        )}
        {(rows.length > 0 || orphanScan.length > 0) && (
          <button className="btn btn-success" style={{ marginLeft: 'auto', padding: '6px 14px', fontSize: 12 }} onClick={exportCSV}>⬇ Export CSV</button>
        )}
      </div>
