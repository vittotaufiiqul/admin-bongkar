...
const TABS = [
  ...
  { id: 'lebihan', label: '🗄️ Lebihan & Rak'   },
...
export default function App() {
  ...
  const loadAll = useCallback(async () => {
    ...
    try {
      const [p, s, m, pl] = await Promise.all([
        ...
        db.getAll('pindah_rak').catch(() => []), // graceful if table not yet created
      ])
  ...
  // Total lebihan badge
  const totalLebihan = scan.reduce((a, s) => {
    const dipindah = pindahList.filter(p => p.sku === s.sku && p.supplier === s.supplier)
      .reduce((x, p) => x + Number(p.qty_pindah), 0)
    return a + Math.max(0, Number(s.qty_lebihan || 0) - dipindah)
  }, 0)

  if (!configured && !ready) return <SetupScreen onDone={() => { resetClient(); setReady(true) }} />
  if (dbStatus === 'loading') return <div style={{ textAlign: 'center', padding: 60, color: '#8892b0', fontFamily: 'monospace' }}>⏳ Memuat data dari database...</div>
      ...
      <div className="tabs">
        {TABS.map(t => (
          <div key={t.id} className={`tab ${tab === t.id ? 'active' : ''}`} onClick={() => setTab(t.id)}>
            ...
            {t.id === 'lebihan' && totalLebihan > 0 && (
              <span className="n-badge" style={{ background: 'var(--orange)' }}>{totalLebihan}</span>
            )}
          </div>
        ))}
      </div>
