  ...
  return (
    <div>
      ...
      <div className="hdr">
        ...
        {/* Desktop tab bar (inside header, hidden on mobile) */}
        <div className="hdr-tabs">
          {TABS.map(t => {
            ...
            return (
              <div key={t.id}
                className={`hdr-tab ${tab===t.id?'active':''}`}
                onClick={()=>setTab(t.id)}>
                {t.icon}
                {t.label}
                {b>0 && (
      ...
      {/* ══ MOBILE BOTTOM TABS ══════════════════════════════════ */}
      {/* Selalu dirender, visibility dikontrol CSS media query   */}
              ...
              <div key={t.id}
                ...
                {b>0 && (
                  <span className="m-badge"
                    style={{ background: red?'var(--red)':t.id==='karung'?'var(--orange)':'var(--brand)' }}>
                    {b>99?'99+':b}
                  </span>
                )}
                {/* Icon */}
