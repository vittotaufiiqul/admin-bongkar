...
const T = {
  ...
  perm:    <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z"/><polyline points="14 2 14 8 20 8"/><line x1="16" y1="13" x2="8" y2="13"/></svg>,
  ...
  antrian: <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><rect x="2" y="7" width="20" height="14" rx="2"/><path d="M16 21V5a2 2 0 0 0-2-2h-4a2 2 0 0 0-2 2v16"/></svg>,
...
export default function App() {
  ...
  const antrianCount = [...new Set(scan.filter(s=>Math.max(0,Number(s.qty_lebihan||0)-(lbhKey[`${s.supplier}__${s.sku}`]||0))>0).map(s=>`${s.supplier}__${s.sku}`))].length
  ...
  if (dbStatus==='loading')              return <Spinner msg="Memuat data..."/>
  ...
  return (
    <div>
        ...
        {TABS.map(t => {
          ...
          return (
            <div key={t.id} className={`desktop-tab ${tab===t.id?'active':''}`} onClick={()=>setTab(t.id)}>
              {T[t.id]} {t.label}
              {b>0 && <span className={`n-badge ${red?'red':''}`}>{b>99?'99+':b}</span>}
            </div>
          )
        })}
      </div>
      ...
      {/* ── Mobile bottom tabs — scrollable ── */}
      <nav className="mobile-tabs" role="navigation">
          ...
          {TABS.map(t => {
            ...
            return (
              <div key={t.id} className={`mobile-tab ${tab===t.id?'active':''}`} onClick={()=>setTab(t.id)}>
                {b>0 && <span className="m-badge" style={{ background:red?'var(--red)':'var(--brand)' }}>{b>99?'99+':b}</span>}
                {T[t.id]}
                <span>{t.label}</span>
              </div>
            )
