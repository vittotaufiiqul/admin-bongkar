...
// SVG tab icons
const TabIcons = {
  scan:    <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><rect x="3" y="3" width="7" height="7"/><rect x="14" y="3" width="7" height="7"/><rect x="14" y="14" width="7" height="7"/><rect x="3" y="14" width="7" height="7"/></svg>,
  perm:    <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z"/><polyline points="14 2 14 8 20 8"/><line x1="16" y1="13" x2="8" y2="13"/><line x1="16" y1="17" x2="8" y2="17"/><polyline points="10 9 9 9 8 9"/></svg>,
  rekap:   <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><polyline points="22 12 18 12 15 21 9 3 6 12 2 12"/></svg>,
  lebihan: <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M21 16V8a2 2 0 0 0-1-1.73l-7-4a2 2 0 0 0-2 0l-7 4A2 2 0 0 0 3 8v8a2 2 0 0 0 1 1.73l7 4a2 2 0 0 0 2 0l7-4A2 2 0 0 0 21 16z"/></svg>,
  stok:    <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><rect x="3" y="3" width="18" height="18" rx="2"/><path d="M3 9h18M9 21V9"/></svg>,
  notif:   <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M18 8A6 6 0 0 0 6 8c0 7-3 9-3 9h18s-3-2-3-9"/><path d="M13.73 21a2 2 0 0 1-3.46 0"/></svg>,
  master:  <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><circle cx="12" cy="12" r="3"/><path d="M19.07 4.93a10 10 0 0 1 0 14.14"/><path d="M4.93 4.93a10 10 0 0 0 0 14.14"/></svg>,
}
...
export default function App() {
  ...
  // Cek session saat pertama buka — tidak query user_profiles
  useEffect(() => {
  ...
  const Spinner = ({ msg }) => (
    <div style={{ display:'flex', flexDirection:'column', alignItems:'center', justifyContent:'center', height:'100vh', gap:16, background:'var(--bg)' }}>
      <div style={{ width:40, height:40, border:'3px solid var(--b2)', borderTopColor:'var(--brand)', borderRadius:'50%', animation:'spin 1s linear infinite' }}/>
  ...
  if (session.profile.role === 'picker') return (
    <>
      <Toasts toasts={toasts}/>
      <PickerView profile={session.profile} onLogout={handleLogout} toast={toast} />
    </>
  )
  if (dbStatus === 'loading')            return <Spinner msg="Memuat data gudang..." />
  ...
  return (
    <div>
      ...
      <div className="desktop-tabs">
        {TABS.map(t => {
          const badge = t.id==='notif'?notifBaru:t.id==='lebihan'?totalLebihan:counts[t.id]
          const red   = t.id==='notif'||t.id==='lebihan'
          return (
      ...
      <div className="mobile-tabs">
        {TABS.map(t=>{
          const b=t.id==='notif'?notifBaru:t.id==='lebihan'?totalLebihan:counts[t.id]
          const red=t.id==='notif'||t.id==='lebihan'
          return (
            <div key={t.id} className={`mobile-tab ${tab===t.id?'active':''}`} onClick={()=>setTab(t.id)}>
              {b>0&&<span className="n-badge" style={{position:'absolute',top:4,right:'15%',fontSize:8,padding:'1px 4px',background:red?'var(--red)':'var(--brand)'}}>{b>99?'99+':b}</span>}
