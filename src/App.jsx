...
// ── Lazy Tab Mount ─────────────────────────────────────────────
// Komponen di-mount saat pertama kali dikunjungi, lalu disembunyikan
// dengan CSS (bukan unmount). Ini mencegah re-render mahal saat pindah tab.
function LazyTab({ id, activeTab, children }) {
  const hasBeenActive = useRef(false)
  if (id === activeTab) hasBeenActive.current = true

  if (!hasBeenActive.current) return null // Belum pernah dibuka — jangan mount
...
export default function App() {
  ...
  const [loadProgress, setLoadProgress] = useState('') // progress text
  ...
  const loadAll = useCallback(async () => {
    setDbStatus('loading')
    setLoadProgress('Mengambil data...')
    const t0 = Date.now()
    try {
      // Semua tabel di-fetch paralel sekaligus
      setLoadProgress('Memuat semua data paralel...')
      ...
      console.log(`[perf] loadAll selesai dalam ${elapsed}s — scan:${s.length} perm:${p.length} master:${m.length}`)
    } catch(e) {
      setDbStatus('error')
      setLoadProgress('')
  ...
  const Spinner = ({ msg, sub }) => (
    <div style={{ display:'flex', flexDirection:'column', alignItems:'center', justifyContent:'center', height:'100vh', gap:14, background:'var(--bg)' }}>
      ...
      {sub && <div style={{ color:'var(--t3)', fontSize:11, fontFamily:'var(--mono)' }}>{sub}</div>}
  ...
  if (dbStatus==='loading')             return <Spinner msg="Memuat data warehouse..." sub={loadProgress}/>

  const { profile } = session

  // Props yang diteruskan ke semua tab
  const tabProps = {
    scan, setScan, perm, master, pindahList, karunginList, notifList,
    addScan, addPerm, addMaster, addPindah, addKarungin,
    delScan, delPerm, delMaster, delKarungin,
    setMaster, setNotifList,
    toast,
  }
    ...
    <div>
      ...
      <div className="hdr">
        ...
        <div className="hdr-right">
          ...
          <button className="btn btn-ghost btn-sm" onClick={loadAll} title="Refresh" disabled={dbStatus==='loading'}>
