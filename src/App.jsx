...
const T = {
  ...
  perm:    <svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z"/><polyline points="14 2 14 8 20 8"/><line x1="16" y1="13" x2="8" y2="13"/></svg>,
  ...
  karung:  <svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M21 16V8a2 2 0 0 0-1-1.73l-7-4a2 2 0 0 0-2 0l-7 4A2 2 0 0 0 3 8v8a2 2 0 0 0 1 1.73l7 4a2 2 0 0 0 2 0l7-4A2 2 0 0 0 21 16z"/><polyline points="3.27 6.96 12 12.01 20.73 6.96"/><line x1="12" y1="22.08" x2="12" y2="12"/></svg>,
...
export default function App() {
  ...
  const [karunginList, setKarunginList] = useState([])   // ← baru
  ...
  const loadAll = useCallback(async () => {
    ...
    try {
      const [p, s, m, pl, nl, kl] = await Promise.all([
        ...
        db.getAll('karungin_lebihan').catch(()=>[]),  // ← baru
      ])
  ...
  const addKarungin  = async r => { const d=await db.insert('karungin_lebihan',r); setKarunginList(p=>[d,...p]) }  // ← baru
  ...
  const delKarungin = async id => { await db.delete('karungin_lebihan',id); setKarunginList(p=>p.filter(r=>r.id!==id)) }  // ← baru

  // ── Badges ─────────────────────────────────────────────────
  const lbhKey = {}
  pindahList.forEach(p  => { const k=`${p.supplier}__${p.sku}`;  lbhKey[k]=(lbhKey[k]||0)+Number(p.qty_pindah) })
  karunginList.forEach(k => { const key=`${k.supplier}__${k.sku}`; lbhKey[key]=(lbhKey[key]||0)+Number(k.qty) })

  // Antrian: lebihan yang belum dipindah atau dikarungin
  ...
  // Karungin badge: jumlah SKU lebihan yang belum dikarungin
  const lebihanBelumKarung = (() => {
    ...
    pindahList.forEach(p  => { const k=`${p.supplier}__${p.sku}`;  map[k]=(map[k]||0)-Number(p.qty_pindah) })
    karunginList.forEach(k => { const key=`${k.supplier}__${k.sku}`; map[key]=(map[key]||0)-Number(k.qty) })
  ...
  return (
    <div>
      ...
      <div className="desktop-tabs">
        {TABS.map(t => {
          const badge = t.id==='antrian' ? antrianBadge : t.id==='karung' ? lebihanBelumKarung : 0
          const red   = t.id==='antrian' && notifBaru>0
          return (
            <div key={t.id} className={`desktop-tab ${tab===t.id?'active':''}`} onClick={()=>setTab(t.id)}>
              <span style={{ opacity:tab===t.id?1:.5 }}>{T[t.id]}</span>
              {t.label}
              {badge>0 && <span className={`n-badge ${red?'red':t.id==='karung'?'orange':''}`}>{badge>99?'99+':badge}</span>}
            </div>
          )
        })}
      </div>
          ...
          {TABS.map(t => {
            ...
            return (
              <div key={t.id} className={`mobile-tab ${tab===t.id?'active':''}`} onClick={()=>setTab(t.id)}>
                {badge>0 && (
                  <span className="m-badge" style={{ background: red?'var(--red)':t.id==='karung'?'var(--orange)':'var(--brand)' }}>
                    {badge>99?'99+':badge}
                  </span>
                )}
                {T[t.id]}
