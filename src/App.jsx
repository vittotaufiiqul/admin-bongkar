...
const TabIcons = {
  ...
  antrian:  <svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><rect x="2" y="7" width="20" height="14" rx="2" ry="2"/><path d="M16 21V5a2 2 0 0 0-2-2h-4a2 2 0 0 0-2 2v16"/></svg>,
...
export default function App() {
  ...
  const antrianCount = scan.filter(s=>Math.max(0,Number(s.qty_lebihan||0)-(lbhKey[`${s.supplier}__${s.sku}`]||0))>0)
    .reduce((set,s)=>{set.add(`${s.supplier}__${s.sku}`);return set},new Set()).size
  const notifBaru = notifList.filter(n=>n.status==='baru').length
  const antrianBadge = antrianCount + notifBaru  // gabungan badge
  ...
  if (session.profile.role === 'picker') return (
    <><Toasts toasts={toasts}/><PickerView profile={session.profile} onLogout={handleLogout} toast={toast}/></>
  )
        ...
        {TABS.map(t => {
          ...
          return (
            <div key={t.id} className={`desktop-tab ${tab===t.id?'active':''}`} onClick={()=>setTab(t.id)}>
              ...
              {t.label}
              {badge > 0 && (
                <span className={`n-badge ${red ? 'orange' : ''}`}>{badge > 99 ? '99+' : badge}</span>
              )}
            </div>
          )
        })}
        ...
        {TABS.map(t => {
          ...
          return (
            <div key={t.id} className={`mobile-tab ${tab===t.id?'active':''}`} onClick={()=>setTab(t.id)}>
              {b > 0 && (
                <span className="n-badge" style={{ position:'absolute', top:4, right:'15%', fontSize:8, padding:'1px 4px', background: red ? 'var(--red)' : 'var(--brand)', color: red ? '#fff' : '#000' }}>
                  {b > 99 ? '99+' : b}
                </span>
              )}
